...
function loadMediaMap(db, ids) {
  ...
  const rows = db
    .prepare(`SELECT id, variants_json, mime_type FROM media WHERE id IN (${placeholders})`)
    .all(...ids);
  return new Map(
    rows.map((r) => [
      r.id,
      { id: r.id, variants: JSON.parse(r.variants_json), mimeType: r.mime_type },
    ]),
  );
}
...
export default function newsRoutes(db) {
  ...
  function uniqueSlug(base, excludeId) {
    ...
    let i = 2;
    for (;;) {
      const row = db.prepare("SELECT id FROM news WHERE slug = ?").get(candidate);
  ...
  router.get("/", requireAuth, (req, res) => {
    ...
    const where = status && STATUSES.includes(status) ? "WHERE status = ?" : "WHERE status != 'deleted'";
    const rows = status && STATUSES.includes(status)
      ? db.prepare(`SELECT * FROM news ${where} ORDER BY COALESCE(publish_at, created_at) DESC`).all(status)
      : db.prepare(`SELECT * FROM news ${where} ORDER BY COALESCE(publish_at, created_at) DESC`).all();
    const mediaIds = rows.filter((r) => r.hero_media_id).map((r) => r.hero_media_id);
  ...
  router.get("/public", (_req, res) => {
    ...
    const mediaMap = loadMediaMap(db, rows.filter((r) => r.hero_media_id).map((r) => r.hero_media_id));
    res.json(rows.map((r) => toPublic(r, mediaMap)));
  });
  ...
  router.get("/public/:slug", (req, res) => {
    ...
    if (!row) return errorEnvelope(res, 404, "not_found", "Beitrag nicht gefunden.");
    const mediaMap = loadMediaMap(db, row.hero_media_id ? [row.hero_media_id] : []);
  ...
  router.post("/", requireAuth, requireCsrf, (req, res) => {
    ...
    if (!baseSlug) return errorEnvelope(res, 400, "bad_request", "Titel darf nicht leer sein.");
    const finalSlug = uniqueSlug(baseSlug);
    const now = new Date().toISOString();
    const publishAt = data.status === "scheduled" || data.status === "published" ? data.publishAt ?? now : null;
    const info = db
      .prepare(
  ...
  router.patch("/:id", requireAuth, requireCsrf, (req, res) => {
    ...
    const mediaMap = loadMediaMap(db, updated.hero_media_id ? [updated.hero_media_id] : []);
  ...
  router.delete("/:id", requireAuth, requireCsrf, (req, res) => {
    ...
    db.prepare("UPDATE news SET status = 'deleted', updated_at = ? WHERE id = ?").run(
